...
```
 ...
 sessions ───────┼─► harness-hook ──HTTP──►                                          │
                 │                          ┌──────────────┐                         │
 spawned agents  │ stream-json (stdout)      │              │   SQLite (~/.harness)   │
 (harness run) ──┼─────────────────────────► │   harnessd   ├──► ledger + event log   │
                 │                          │              │                         │
...
One long-running local process, **`harnessd`**, owns the database and is the only writer. Everything else is a client: the CLI, the MCP server, the hook shim, and the web UI. If the daemon is not running, clients start it (Unix socket + TCP on localhost; port recorded in `~/.harness/daemon.json`).

## Repo-agnostic model

Harness keeps **no state in the target repository**. Identity and state:

- **Project identity** = realpath of the git *common dir* (`git rev-parse --git-common-dir`), so every worktree of a repo maps to the same project. Non-git folders use the folder realpath. A stable `project_id` (hash) is derived from it; the human name defaults to the folder name and can be renamed in Harness.
- **Registration** is optional. `harness add <path>` registers a project for the dashboard sidebar. Unregistered sessions are still ingested (auto-registered as "discovered") unless the user sets `discover: false`.
- **State** lives in `~/.harness/harness.db` (SQLite, WAL). One DB, all projects. Per-project logical partitions, never per-project files in the repo.
- **Per-repo config** (`.harness.toml` at repo root) is *optional* and read-only for Harness: task source path, resource names, rule overrides, worktree dir. When absent, sensible defaults apply (`.worktrees/` next to the repo, resources `web`, `worker`, `db`). The author's two repos will use it; a fresh repo needs nothing.
- **Hook installation** is at the *user* level (`~/.claude/settings.json`), so every Claude Code session on the machine reports in, whichever folder it runs from. `harness install` adds the hook entries and the MCP server entry; `harness uninstall` removes exactly those. Project-level installation is supported for people who want to opt in one repo.
...
- **Process registry**: pids, ports, cwd, owning session, for anything started through `harness serve` / `harness proc`. Liveness checked by pid + start time, never by command pattern.
...
Stdio MCP server exposing `harness.status`, `harness.claim`, `harness.renew`, `harness.handoff`, `harness.resume`, `harness.release`, `harness.resource.acquire|release`, `harness.gate.record`, `harness.note`. Forwards to the daemon. Auto-detects project from `cwd`. Ported from the Brainstorm dev MCP server.

### `harness-hook` (hook shim)
...
**Agent wants a task** → calls `harness.claim {task:"M0.6"}` → daemon checks ledger, creates `.worktrees/m0.6` on `task/m0.6`, records claim with TTL, returns path → agent `cd`s. Renewals are automatic on any hook event from a session that holds a claim (no more forgetting `renew`).

**Owner spawns a worker from the dashboard** → `harness run --project los --task M0.7 --prompt …` → daemon claims, creates worktree, spawns `claude -p` with stream-json, ingests events, permission prompts route to the rule engine then to the dashboard if unresolved → owner sees live stream, can type into stdin, can stop.
