...
- **Per-repo config** (`.swarm.toml` at repo root) is *optional* and read-only for Swarm: task source path, resource names, rule overrides, worktree dir. When absent, sensible defaults apply (`.worktrees/` next to the repo, resources `web`, `worker`, `db`). Projects that want it opt in; a fresh repo needs nothing.
...
**Interactive session starts in `~/code/web-app`** → `SessionStart` hook → daemon resolves project by git common dir → session row created → SSE `session.started` → dashboard shows it under web-app. Every tool call arrives as `PreToolUse`; rule engine checks "is this an `Edit|Write` under the shared tree while the project has `shared_tree_readonly`?" → deny with the reason text the agent sees. Hook response also injects the agent's current claim into context on `SessionStart`/`UserPromptSubmit`.
