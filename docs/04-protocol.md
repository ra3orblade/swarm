...
`harness run` spawns `claude -p --output-format stream-json --input-format stream-json --verbose [--include-partial-messages]`, tags every line with `session_id` from the `system/init` event, and ingests `assistant`, `user`, `stream_event`, `result`. Permission prompts go through `--permission-prompt-tool mcp__harness__permission`, which calls the same rule engine as path A. stdin is held open; `POST /v1/sessions/:id/input` writes a user message to it.

### C. MCP tools (agent self-service)
`harness-mcp` on stdio, registered user-wide. Tools map 1:1 to daemon endpoints. The server resolves the project from its own `cwd`, so the same config works in every repo.

## Egress

- **SSE** `GET /v1/events?project=&session=&since=` — the normalized event stream, replayable by `seq`. The dashboard and `harness tail` use this.
...
- **Unix socket** `~/.harness/harnessd.sock` — same HTTP, no port discovery needed for local clients.
...
Claude Code's hook and stream-json schemas drift. `packages/core` pins an `adapters/claude-code` module with the parsing, and `harness doctor` reports the installed `claude --version` against the tested range. The rest of the system only sees normalized events.
