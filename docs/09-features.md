...
Every group above ships today. **v0.14.0 is the Act release** (decided 2026-09-12): interactive sessions get the authority spawned runs already have — a repair loop that refuses a stop until the gates pass, the permission card on interactive prompts, live collision warnings, messages that wake the session, rules that rewrite a call instead of denying it, and Codify that opens the PR — plus a statusline and plan quota windows from M12, and a team daemon a person can set up without a clone or a terminal (published package and image, `swarm-teamd setup`, members / machines / policy in the team dashboard, a Team panel in the local one). That is **M13 Act** (M13.1–M13.6, M13.12) with M12.2–M12.3 in the [roadmap](06-roadmap.md); decisions OQ-24 – OQ-27.
