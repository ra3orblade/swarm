...
Every group above ships today. **v0.14.0 is the Act release** (decided 2026-09-12): interactive sessions get the authority spawned runs already have — a repair loop that refuses a stop until the gates pass, the permission card on interactive prompts, live collision warnings, messages that wake the session, rules that rewrite a call instead of denying it, and Codify that opens the PR — plus a statusline and plan quota windows from M12, and **Host a team** / **Join a team** buttons in the app (the team daemon fetched and run by the app, LAN discovery, invite link, members / machines / policy in the same panel; a published package and image for teams that start on a server). That is **M13 Act** (M13.1–M13.6, M13.12) with M12.2–M12.3 in the [roadmap](06-roadmap.md); decisions OQ-24 – OQ-27.

**M12 Reach** (from the 2026-09-06 landscape survey in [15-landscape](15-landscape.md)) continues in 0.15: OpenTelemetry export, rules on Codex / Gemini CLI / Cursor, a destructive-command family with secrets and tamper guards, sandboxed runs, a reviewer on permission prompts, user-defined signals and a score, cost by MCP server, and CI feedback into the run — decisions OQ-20 – OQ-23 in [open questions](07-open-questions.md) — alongside the rest of M13 (worktrees Claude Code creates, guards at the moment of a model switch or config change, memory across compaction, scheduled workflows, findings that become issues).
