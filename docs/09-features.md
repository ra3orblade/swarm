...
- **Runtime resources** — ports, workers, databases as named singletons so two agents can't corrupt shared state; processes tracked by pid, never killed by command pattern.
