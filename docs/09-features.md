...
Every group above ships today. **v0.14.0 is the Act release** (2026-09-12): interactive sessions get the authority spawned runs already had — a repair loop that refuses a stop while a required gate fails, the permission card on interactive prompts, live collision warnings, messages that wake an idle session, rules that rewrite a call instead of denying it (with `[[rules.custom]]`), and Codify that opens a PR — plus a status line inside Claude Code, plan quota windows, and **Host a team** / **Join a team** in the dashboard. That is **M13 Act** (M13.1–M13.6, M13.12) with M12.2–M12.3 in the [roadmap](06-roadmap.md); decisions OQ-24 – OQ-27. Still open from that milestone: publishing the team daemon and finishing its own Settings page (M13.13), and the rest of M12 and M13.
