...
- **Lineage** — who started whom over the last 14 days: subagents, dispatched runs, messages between agents, and handoffs from one claim holder to the next. Four relationships Swarm already records, drawn as one graph, newest at the top; a parent sits level with the first thing it started. A green pill is a collapsed group (its tooltip carries what the group cost), a ring is the branch's outcome once it has a PR, and a bowed edge closed a loop (two agents messaging each other is legitimate, so nothing pretends the graph is acyclic). The picture is capped at the 40 most recently related sessions, and the heading says how many older ones it left out.
