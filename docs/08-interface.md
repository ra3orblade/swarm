...
| `GET /v1/projects` · `POST /v1/projects` · `PATCH /v1/projects/:id` · `DELETE /v1/projects/:id` | register (pin) / unpin a project; `PATCH` takes `{name?, pinned?, icon?, color?}` — **project settings** (sidebar menu → Settings…): `icon` is an emoji or ≤ 4-char glyph shown instead of the folder icon, `color` a categorical slot `c1`…`c7` (a design token) tinting the project glyph everywhere; `POST` with `{path}` is idempotent and returns the project id |
