...
| M1.6 | Claims board in the dashboard (lease countdown, orphan highlighting, release/force-release); `.swarm.toml` task source deferred | M1.1 | 🟡 2026-08-22 Claims section on the **Board** view (moved from Fleet): held/expired/orphaned with lease countdown, release + force-release actions, orphans surfaced first; task-source parsing still ⚪ |
...
| ID | Task | Depends | Status |
|----|------|---------|--------|
| M2.1 | Rule engine + built-ins: `shared-tree-readonly`, `no-pattern-kill`, `claim-required-to-write`, `protected-ports`, `no-foreign-worktree` | M1 | 🟡 2026-08-22 rules v2: `shared_tree` / `destructive_git` / `pattern_kill` / `protected_ports` in `core/rules.ts`, each configurable per repo as `ask` / `deny` / `off` (13-config); held resource ports auto-protected; `deny` returned to Claude Code as a real permission denial, `ask` as a confirmation; `claim-required-to-write` and `no-foreign-worktree` still ⚪ |
