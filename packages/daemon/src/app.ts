...
export function createApp(store = new Store(), hooks: { restart?: () => void } = {}) {
  ...
  // The React dashboard (M11), served beside the original rather than replacing it: the two cannot
  // share `#main`, and the working dashboard has to keep working while this one is built. It takes
  // over `/` when the last view is ported (M11.12).
  app.get("/next", (c) => {
    const p = join(WEB_DIR, "next.html");
    if (!existsSync(p)) return c.text("next.html not built — run: bun run build:web", 404);
