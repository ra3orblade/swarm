...
export const VERSION = process.env.SWARM_VERSION ?? "0.13.0";
export { Store };
...
export function createApp(store = new Store(), hooks: { restart?: () => void } = {}) {
  ...
  // `/next` was where the React dashboard lived while it was being built (M11). It now serves `/`,
  // so the alias only exists to keep an old bookmark working.
  app.get("/next", (c) => c.redirect("/", 308));
  // Static assets, generated by `bun run build:web`: dashboard.js + dashboard.css (the app),
  // menus.js, fm.css, icons.js. `viz.js` and `table.js` also live here, but only because the team
  // dashboard (packages/team) is still a vanilla page and loads them from this directory.
  // Flat directory, fixed extension set — no path traversal.
  ...
  // Images are binary: read them as bytes, never as UTF-8 (which would mangle every one of them).
  ...
  app.get("/:file{[a-z0-9-]+\\.(js|css|svg|ico|png)}", (c) => {
    ...
    // browser was free to keep the previous build's dashboard.js and icons.js after the daemon
    ...
    return c.body(BINARY.has(ext) ? readFileSync(p) : readFileSync(p, "utf8"), 200, {
