...
**`role="button"` is allowed in exactly three files.** `Dag.tsx`, `Timeline.tsx` and `Sidebar.tsx`
turn `useSemanticElements` off: SVG has no `<button>` (a graph node can only be an operable `<g>`),
a timeline row is a pair of CSS-grid cells a `<button>` would break, and a project row *contains*
the `⋯` menu button — nesting one button inside another is invalid HTML and the click gets
swallowed. All three carry `tabIndex`, an `aria-label` and an Enter/Space handler. Nowhere else
should need it.
